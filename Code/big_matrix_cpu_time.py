...
def IRWA(H, g, A, B, cons_numb, x_0, step_t, lamb, gamma, Varpsilon, eta, delta, M, N): 
    ...
    for i in range(N):
        ...
        time_inital = time_inital + time_temp
        Time.append(time_inital)
        ...
        if np.linalg.norm(x_next-x_t, ord=2) < delta[0] and np.linalg.norm(Varpsilon, ord=2) < delta[1]:
            return x_next, X, obj_value, Time
        x_t = x_next
        #print("step_t:", step_t)
    return x_t, X, obj_value, Time
...
def QP_problem(cons_numb, m, n, x_0, step_t, lamb, gamma, Varpsilon, eta, delta, tau, theta, M, N): 
    ...
    #problem solving
    t_start=time()
    x_sol, X, obj_value, Time = IRWA(H, g, A, B, cons_numb, x_0, step_t, lamb, gamma, Varpsilon, eta, delta, M, N)
    t_end=time()
    time_1 = t_end-t_start
    print("time1 :", time_1)
    t_start=time()
    x_sol_2, X_2, obj_value_2, Time_2 = IRWA_accelerate(H, g, A, B, cons_numb, x_0, step_t, lamb, gamma, Varpsilon, eta, delta, tau, theta, M, N)
    t_end=time()
    ...
    return x_sol, f_min, X, obj_value, Time, x_sol_2, f_min_2, X_2, obj_value_2, Time_2
...
theta = 0.00001
t_start=time()
x_sol, f_min, X, obj_value, Time, x_sol_2, f_min_2, X_2, obj_value_2, Time_2 = QP_problem(N, m, n, x_0, 1, lamb, gamma, Varpsilon, eta, delta, tau, theta, 100, 5000)#step_t is defined in IRWA
...
plt.plot(Time, obj_value, label="IRWA")
plt.plot(Time_2, obj_value_2, label="AIRWA")
plt.legend()
plt.title('cpu运行时间', fontproperties = font_properties)
